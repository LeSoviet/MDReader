...
if (Test-Path $7zPath) {
    ...
    & $7zPath a "dist\MDReader-1.2.0.zip" "dist\MD Reader-Setup-1.2.0.exe" "dist\README.md" "dist\LICENSE" -r
} else {
    # Use PowerShell's built-in compression
    Write-Host "Using PowerShell compression..." -ForegroundColor Yellow
    $filesToCompress = Get-ChildItem "dist" -Include "MD Reader-Setup-1.2.0.exe", "README.md", "LICENSE"
    if ($filesToCompress.Count -gt 0) {
        Compress-Archive -Path $filesToCompress -DestinationPath "dist\MDReader-1.2.0.zip" -Force
    }
}

# Clean up copied files
Remove-Item "dist\MD Reader-Setup-1.2.0.exe" -ErrorAction SilentlyContinue
